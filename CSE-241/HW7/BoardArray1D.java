/**
* <h1> BoardArray1D subclass for N- Puzzle Problem </h1>
...
public class BoardArray1D extends AbstractBoard
{
	...
	*/
	public BoardArray1D(int val_i, int val_j)
	{
		super(val_i, val_j);
		generateMap();
		shuffle(50);
	...
	*/
	BoardArray1D(AbstractBoard copyObject)
	{
		...
		for(int i = 0 ; i < val_i ; ++i)
			for(int j = 0 ; j < val_j ; ++j)
				gameBoard[(i*val_j) + j] = copyObject.cell(i, j);
	}
	...
	*/
	...
	{
		for(int i = 0 ; i < this.getSize_i() * this.getSize_j() ; ++i)
			gameBoard[i] = -1;
	}
	...
	*/
	private int findMax()
	{
		...
		for(int i = 0 ; i < this.getSize_i() * this.getSize_j() ; ++i, ++index)
			if(max < this.gameBoard[index])
				max = gameBoard[index];
	...
	public char move(char choice)
	{
		if(choice == 'L' || choice == 'l')
		{
			...
			if(cell__j != 0) // condition left
			{
				i = cell__i;
				j = cell__j;
				temp = this.gameBoard[(size__j*cell__i) + (cell__j-1)];
				this.gameBoard[(size__j*i) + (j-1)] = 0;
				this.gameBoard[(size__j*i) + j] = temp;
		...
		else if(choice == 'R' || choice == 'r')
		{
			...
			if(cell__j < (this.getSize_j() - 1)) // condition right
			{
				i = cell__i;
				j = cell__j;
				temp = this.gameBoard[(size__j*i) + (j+1)];
				this.gameBoard[(size__j*i) + (j+1)] = 0;
				this.gameBoard[(size__j*i) + j] = temp;
		...
		else if(choice == 'U' || choice == 'u')
		{
			...
			if(cell__i != 0) // condition up
			{
				i = cell__i;
				j = cell__j;
				temp = this.gameBoard[(size__j*i) + (j-size__j)];
				this.gameBoard[(size__j*i) + (j-size__j)] = 0;
				this.gameBoard[(size__j*i) + j] = temp;
		...
		else if(choice == 'D' || choice == 'd')
		{
			...
			if(cell__i < (this.getSize_i() - 1)) // condition right
			{
				i = cell__i;
				j = cell__j;
				temp = this.gameBoard[(size__j*i) + (j+size__j)];
	...
	*/
	public int cell(int index_i, int index_j)
	{
		...
		return (this.gameBoard[(index_i * this.getSize_j()) + index_j]);
	}
		...
		try
		{
			...
			for(int ii = 0 ; ii < this.getSize_i() ; ++ii)
				for(int jj = 0 ; jj < this.getSize_j() ; ++jj)
					gameBoard[(ii*this.getSize_j()) + jj] = -1;
